...
## 五、TROUBLESHOOTING（常见坑排查）

> 针对规范落地时的常见错漏，按「问题 → 原因 → 解决」排查。规范类坑不涉及环境操作，重点在**复核**。

| 问题 | 原因 | 解决 |
|---|---|---|
| 版本号升错位 | 破坏性变更没标 `!` 或 `BREAKING CHANGE:`，或把 `feat` 当 `fix` | 先 `git log` 复核提交类型，再按 MAJOR/MINOR/PATCH 规则回改版本号 |
| `[Unreleased]` 丢失 | 归档版本条目后忘了重建 | 发版步骤第 4 步：归档后立即在顶部补 `## [Unreleased]` |
| 版本条目日期格式错 | 日期写了本地格式 | 用 ISO 8601：`## [1.2.3] - 2026-08-04`，方括号+连字符+`-` |
| changelog 顺序乱 | 新版本插到了中间 | 永远最上方是 `[Unreleased]`，其余按时间倒序 |
| 六分类中文/英文混用 | 标题中英夹杂、不统一 | 用「`### 变更 / Changed`」双语标题，空分类直接省略 |
| 改动类别放错分区 | `feat` 记进 Fixed、`refactor` 记进 Added | 对照「提交类型 → Changelog 分类」表核对 |
| 测试/正式版本号混淆 | 正式版累加了第 4 位、或测试包升了第 3 位 | 测试包累加第 4 位 `0.4.4.x`，正式版才升第 3 位 `0.4.5` |
| 忘了更新版本对比链接 | 文件底部对比链接仍指向旧版 | 发版步骤第 7 步：归档后同步更新底部链接 |
| git 标签与版本号不一致 | 标签打错（如 `1.2.3` 而非 `v1.2.3`） | 统一标签格式 `git tag vX.Y.Z`；打错用 `git tag -d vX.Y.Z` 删除后重打 |
